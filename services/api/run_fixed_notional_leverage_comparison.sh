...
for tier in "${TIERS[@]}"; do
  ...
  printf "%-4s  %-14s  %-10s  %-10s  %-4s  %-10s  %-10s  %-10s  %-10s  %-24s\n" \
    "Lev" "Status" "FeeUsdc" "Premium" "Days" "Selected" "Plan" "Option" "TopLevel" "Reason"
  printf "%s\n" "--------------------------------------------------------------------------------------------------------------"
  ...
  for lev in "${LEVERAGES[@]}"; do
    ...
    printf "%-4s  %-14s  %-10s  %-10s  %-4s  %-10s  %-10s  %-10s  %-10s  %-24s\n" \
      "${lev}" "${status}" "${fee_usdc}" "${premium}" "${days_out}" "${selected}" "${plan_venue}" "${option_venue}" "${top_venue}" "${reason}"
  done
