...
const findBestPut = async (asset, spot, drawdownPct, targetDays) => {
...
const run = async () => {
  ...
  for (const asset of ASSETS) {
    ...
    for (const tier of TIERS) {
      const best3 = await findBestPut(asset, spot, tier.drawdownPct, 3);
      const best4 = await findBestPut(asset, spot, tier.drawdownPct, 4);
      perTier.push({ tier: tier.name, spot, best3, best4 });
    }
  ...
  for (const asset of results) {
    ...
    for (const tierInfo of asset.perTier) {
      ...
      const logEntry = (label, best) => {
        if (best.status !== "ok") {
          console.log(`  ${tierInfo.tier} (${label}): status=${best.status}`);
          return;
        }
        console.log(
          `  ${tierInfo.tier} (${label}): expiry=${best.expiryTag} strike=${best.strike} ask=${best.ask} spread=${(
            best.spread * 100
          ).toFixed(2)}% fee=${fee}`
        );
      ...
      logEntry("3d", tierInfo.best3);
      logEntry("4d", tierInfo.best4);
    }
  }
};
