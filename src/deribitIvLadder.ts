...
export function createDeribitIvLadderCache(
  ...
  const priceBufferPct = options.priceBufferPct ?? 0.02;
  ...
  const buildLadder = async (): Promise<LadderLeg[]> => {
    ...
    const puts = results.filter(
      (inst: any) => inst.option_type === "put" && inst.expiration_timestamp
    );
    ...
    for (const tag of expiryTags) {
      ...
      const tenorDays = Math.max(
        1,
        Math.round((pool[0].expiration_timestamp - now) / (24 * 60 * 60 * 1000))
      );
  ...
  const getSnapshot = (): LadderSnapshot | null => {
    ...
    if (values.length < 3) {
      if (lastSnapshot && now - lastSnapshot.ts <= maxSnapshotAgeMs) {
        return lastSnapshot;
      }
      return null;
    }
    ...
    lastSnapshot = {
      ...
      hedgeIv,
      ts: now,
      instruments: ladderInstruments.slice(),
      legs,
      spot: lastSpot
    };
