...
total_entries="$(echo "${logs}" | jq '.entries | length')"
eval_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="intermittent_hedge_eval")] | length')"
eval_live_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="intermittent_hedge_eval") | select(.payload.selectionMode=="live")] | length')"
eval_shadow_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="intermittent_hedge_eval") | select(.payload.selectionMode=="shadow")] | length')"
selection_error_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="intermittent_hedge_eval") | select(.payload.selectionError!=null)] | length')"
hedge_action_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="hedge_action")] | length')"
hedge_skip_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="hedge_action_skipped")] | length')"
close_blocked_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="close_blocked")] | length')"
demo_credit_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="demo_credit")] | length')"
mtm_credit_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="mtm_credit")] | length')"
mtm_position_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="mtm_position")] | length')"
low_ratio_count="$(echo "${logs}" | jq --argjson min "${MIN_COVERAGE_RATIO}" '[.entries[] | select(.event=="mtm_position") | .payload.coverageRatio | tonumber?] | map(select(.!=null and . < $min)) | length')"
negative_buffer_count="$(echo "${logs}" | jq '[.entries[] | select(.event=="mtm_position") | .payload.drawdownBufferUsdc | tonumber?] | map(select(.!=null and . < 0)) | length')"
...
echo "${logs}" | jq -r '[.entries[] | select(.event=="hedge_action_skipped") | .payload.reason] | group_by(.) | map({reason: .[0], count: length}) | sort_by(-.count) | .[0:10]'
echo

echo "Recent intermittent_hedge_eval (latest 5):"
echo "${logs}" | jq -r '[.entries[] | select(.event=="intermittent_hedge_eval")] | .[0:5]'
echo
